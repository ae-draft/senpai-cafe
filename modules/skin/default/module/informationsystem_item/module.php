...
class Skin_Default_Module_Informationsystem_Module extends Informationsystem_Module
{
	 ...
	 * @var string
	 */
	protected $_moduleName = 'informationsystem_items';
	...
	{
		$iInformationsistemId = 3;
		$oInformationsystem_Item = Core_Entity::factory('Informationsystem_Item', $iInformationsistemId);
		...
		$windowId = 'modalInformationsystemItems';
		$shortcutImg = "/modules/skin/{$this->_skinName}/images/module/{$this->_moduleName}.png";
		$shortcutTitle = 'Гостевая книга';

		if (count($aInformationsystem_Items) > 0)
		{
			if (!$ajax)
			{
			...
			foreach ($aInformationsystem_Items as $key => $oSiteuser)
			{
