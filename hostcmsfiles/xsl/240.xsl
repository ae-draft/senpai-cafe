...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/informationsystem">
		...
		<!--<xsl:if test="group = 0">
			...
			&lt;!&ndash; Описание выводится при отсутствии фильтрации по тэгам &ndash;&gt;
		...
		<xsl:if test="$items_count &gt; $limit and ($page + $post_count_page + 1) &gt; $i">
			...
			<!--<xsl:if test="$i != $page">
				<xsl:if test="($page - $pre_count_page) &lt;= $i and $i &lt; $n">
					&lt;!&ndash; Выводим ссылки на видимые страницы &ndash;&gt;
					<a href="{$group_link}{$number_link}{$tag_path}" class="page_link">
						<xsl:value-of select="$i + 1"/>
					</a>
				</xsl:if>
				
				&lt;!&ndash; Выводим ссылку на последнюю страницу &ndash;&gt;
