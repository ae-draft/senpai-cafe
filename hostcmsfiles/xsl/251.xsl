...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/informationsystem">
		...
		<!-- Если находимся в корне - выводим название информационной системы -->
		<xsl:choose>
		...
		<h2>Метка — <strong><xsl:value-of select="tag/name" disable-output-escaping="yes" /></strong>.</h2>
	...
	<xsl:template match="informationsystem_item">
		<div style="margin-right: 10px; margin-bottom: 20px;">
			...
			<img src="/hostcmsfiles/images/comments.gif" /><span><a href="{url}#comments"><xsl:value-of select="comments_count"/><xsl:text> </xsl:text><xsl:call-template name="declension"> <xsl:with-param name="number" select="comments_count"/></xsl:call-template></a></span>
					</xsl:if>
				</p>
			</xsl:if>
			<div style="clear: both;"></div>
