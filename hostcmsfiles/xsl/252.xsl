	...
	<xsl:template match="/informationsystem/informationsystem_item">
				...
				<xsl:when test="image_large !=''">
					<div id="gallery">
						<a href="{dir}{image_large}" target="_blank">
							<img align="left" src="{dir}{image_small}" class="news_img"/>
						</a>
					</div>
				</xsl:when>
				<xsl:otherwise>
					<img align="left" src="{dir}{image_small}" style="margin-right: 10px; margin-bottom: 10px"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
		...
		<p class="tags">
			...
			<xsl:if test="count(tag) &gt; 0">
				<img src="/hostcmsfiles/images/tags.gif" /><span><xsl:apply-templates select="tag"/></span>
			</xsl:if>
			
			<xsl:if test="count(siteuser) &gt; 0">
			<img src="/hostcmsfiles/images/user.gif" /><span><a href="/users/info/{siteuser/login}/"><xsl:value-of select="siteuser/login"/></a></span>
			</xsl:if>
			
			<!-- Дата информационного элемента -->
			<img src="/hostcmsfiles/images/calendar.gif" /> <xsl:value-of select="date"/>, <span hostcms:id="{@id}" hostcms:field="showed" hostcms:entity="informationsystem_item"><xsl:value-of select="showed"/></span>
			<xsl:text> </xsl:text>
		...
		<xsl:if test="/informationsystem/show_comments/node() and /informationsystem/show_comments = 1">
			...
			<p class="title"><a name="comments"></a>Комментарии</p>
				...
				<xsl:if test="//captcha_id != 0 and /informationsystem/siteuser_id = 0">
					<div class="row">
						...
						<div class="field">
							...
							<div class="captcha">
								<img src="/hostcmsfiles/images/refresh.gif" /> <span onclick="$('#comment_{$id}').updateCaptcha('{//captcha_id}{$id}', 30); return false">Показать другое число</span>
							</div>
						</div>
					</div>
