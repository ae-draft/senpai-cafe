...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="informationsystem_item">
		<xsl:if test="position() != 1 and (position()-1) mod 2 =0">
			<xsl:variable name="div_class_name"><xsl:choose>
					<xsl:when test="(position()+1) mod 4 = 0">bg2</xsl:when>
					<xsl:otherwise>bg1</xsl:otherwise>
			</xsl:choose></xsl:variable>
			<xsl:text disable-output-escaping="yes">
				&lt;/div&gt;
			&lt;div class="clearfix </xsl:text>
			<xsl:value-of select="$div_class_name" />
			<xsl:text disable-output-escaping="yes">"&gt;</xsl:text>
		</xsl:if>
		<xsl:variable name="grid_class_name"><xsl:choose><xsl:when test="(position()-1) mod 2 = 0">grid_12</xsl:when>
		<xsl:otherwise>grid_11</xsl:otherwise></xsl:choose></xsl:variable>
		<div class="{$grid_class_name}">
			<div class="wrapper">
				<xsl:if test="image_large != ''">
					<img src="{dir}{image_large}" class="img-indent"/>
				</xsl:if>
				<h6>
			...
			<div class="alignright"><a class="button" href="{url}">Подробнее</a></div>
			</div>
		</div>
	</xsl:template>
